...
$postid = isset($_GET['nid']) ? intval($_GET['nid']) : 0;
  ...
  <div class="container">
      ...
      <div class="col-md-8">
        ...
        ?>
          ...
          <div class="card mb-4">
            ...
            <?php
                $total = mysqli_query($con,"SELECT COUNT(*) as count from tblcomments WHERE postId='$pid'");     
                $total = mysqli_fetch_assoc($total);
                
                $positive = mysqli_query($con,"SELECT COUNT(*) as count from tblcomments WHERE postId='$pid' AND algo_status ='Positive'");
                $positive = mysqli_fetch_assoc($positive);
                
                $negative = mysqli_query($con,"SELECT COUNT(*) as count from tblcomments WHERE postId='$pid' AND algo_status ='Negative'");
                $negative = mysqli_fetch_assoc($negative);

                $netrual = mysqli_query($con,"SELECT COUNT(*) as count from tblcomments WHERE postId='$pid' AND algo_status not in ('Positive','Negative')");
                $netrual = mysqli_fetch_assoc($netrual);
              ?>

            <a class="badge bg-info text-decoration-none link-light" style="color:#fff">Total : <?php echo htmlentities($total['count']); ?></a>
            <a class="badge bg-success text-decoration-none link-light" style="color:#fff"> Positive : <?php echo htmlentities($positive['count']); ?></a>
            <a class="badge bg-danger text-decoration-none link-light" style="color:#fff">Negative : <?php echo htmlentities($negative['count']); ?></a>
            <a class="badge bg-secondary text-decoration-none link-light" style="color:#fff">Neutral : <?php echo htmlentities($netrual['count']); ?></a>
                  <hr/>
    ...
    <div class="row" style="margin-top: -8%">
      <div class="col-md-8">
        ...
        <?php
        $sts = 1;
        $query = mysqli_query($con, "select name,comment,postingDate,algo_status from  tblcomments where postId='$pid' and status='$sts'");
        while ($row = mysqli_fetch_array($query)) {
        ?>
          <div class="media mb-4">
            ...
            <div class="media-body">
              <h5 class="mt-0"><?php echo htmlentities($row['name']); ?> <br />
                ...
                <?php
                if ($row['algo_status'] == 'Positive') { ?>
                  <a class="badge bg-success text-decoration-none link-light" style="color:#fff"><?php echo htmlentities($row['algo_status']); ?></a>
                <?php
                }
                ?>

                <?php
                if ($row['algo_status'] == 'Negative') { ?>
                  <a class="badge bg-danger text-decoration-none link-light" style="color:#fff"><?php echo htmlentities($row['algo_status']); ?></a>
                <?php
                }
                ?>
